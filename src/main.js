 ...
 * 待添加：textShadow 字体阴影
*/
function Danmu({text, start, duration = 3000, color = '#fff', fontFamily = 'serif'}){
...
*/
function Div({element = document.querySelector('#danmu'), DanmuArr, opacity = 1}){
    ...
    this.timeline = Timeline(parseInt(this.DanmuArr.slice(-1)[0].start));

    var tracks = document.querySelectorAll('.rows'); // 后期可能需要用js生成

    this.render = function(sortedDanmuArray = DanmuArr){
        ...
        var renderSingleDanmu = function(DanmuItem, row){
            ...
            `color : ${DanmuItem.color}; font-family : ${DanmuItem.fontFamily};`;  //其他属性

            // 确定位置
            DanmuContainer.style.left = div.clientLeft + div.clientWidth + 'px';
            // 速度值仍不准
            var speed = (div.clientLeft + div.clientWidth - div.clientLeft - 
                DanmuContainer.clientWidth) / DamuItem.duration;
        ...
        var timelineId = setInterval(function(){
            let danmuPerRow;
            while(timeline.current > sortedDanmuArray[index]){
                for(let i of tracks){
